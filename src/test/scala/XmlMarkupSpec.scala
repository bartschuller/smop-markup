...
class XmlMarkupSpec extends Specification {
  "The XmlMarkup parser" should {
    ...
    "parse empty elements" in {
      parse("<foo/>") should beRight(List(MElement("foo", Map.empty, false, Nil)))
      parse("<foo/><bar/>") should beRight(List(MElement("foo", Map.empty, false, Nil), MElement("bar", Map.empty, false, Nil)))
      parse("<foo></foo>") should beRight(List(MElement("foo", Map.empty, false, Nil)))
    }
    "parse nonempty elements" in {
      parse("<a><b/></a>") should beRight(List(MElement("a", Map.empty, false, List(MElement("b", Map.empty, false, Nil)))))
    }
    "parse placeholders in mixed content" in {
      parse(s"<a/>.$ph") should beRight(List(MElement("a", Map.empty, false, Nil), MText("."), MPlaceholder))
    }
    "parse attributes" in {
      parse("""<a a="a a"/>""") should beRight(List(MElement("a", Map("a"->"a a"), false, Nil)))
      parse("""<a a='a"&apos; a' >a</a>""") should beRight(List(MElement("a", Map("a"->"a\"' a"), false, List(MText("a")))))
      parse("""<a a=">" b=''/>""") should beRight(List(MElement("a", Map("a"->">", "b"->""), false, Nil)))
      parse("""<a a="<"/>""") should beLeft
    }
    "parse placeholders for attribute values" in {
      parse(s"""<a a=$ph />""") should beRight(List(MElement("a", Map("a"->MPlaceholderString), false, Nil)))
    }
